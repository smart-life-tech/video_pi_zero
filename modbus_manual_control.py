...
def read_coils(client, start=0, count=5):
    """Read coil states"""
    result = client.read_coils(address=start, count=count, slave=MODBUS_UNIT_ID)
    if result.isError():
...
def write_coil(client, coil_address, value):
    """Write a single coil"""
    result = client.write_coil(address=coil_address, value=value, slave=MODBUS_UNIT_ID)
    if result.isError():
...
def write_multiple_coils(client, start_address, values):
    ...
    result = client.write_coils(address=start_address, values=values, slave=MODBUS_UNIT_ID)
    if result.isError():
